...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.admin-configs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="grupo">{{ trans('cruds.adminConfig.fields.grupo') }}</label>
                <input class="form-control {{ $errors->has('grupo') ? 'is-invalid' : '' }}" type="text" name="grupo" id="grupo" value="{{ old('grupo', 'Global') }}" required>
                @if($errors->has('grupo'))
