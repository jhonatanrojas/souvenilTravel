...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.enlace.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.enlaces.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo" id="grupo">
                    ...
                    @foreach(App\Models\Enlace::GRUPO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('grupo', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('grupo'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orden') ? 'is-invalid' : '' }}" type="number" name="orden" id="orden" value="{{ old('orden', '') }}" required>
                @if($errors->has('orden'))
                    <div class="invalid-feedback">
                        {{ $errors->first('orden') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icono') ? 'is-invalid' : '' }}" type="text" name="icono" id="icono" value="{{ old('icono', '') }}" required>
                @if($errors->has('icono'))
                    <div class="invalid-feedback">
                        {{ $errors->first('icono') }}
                    </div>
