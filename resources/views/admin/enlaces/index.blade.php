...
@can('enlace_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.enlaces.create') }}">
                ...
                <tbody>
                    @foreach($enlaces as $key => $enlace)
                        <tr data-entry-id="{{ $enlace->id }}">
                            ...
                            <td>
                                @can('enlace_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.enlaces.show', $enlace->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('enlace_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.enlaces.edit', $enlace->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('enlace_delete')
                                    <form action="{{ route('admin.enlaces.destroy', $enlace->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('enlace_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.enlaces.massDestroy') }}",
