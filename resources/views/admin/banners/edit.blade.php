    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.banners.update", [$banner->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('target') ? 'is-invalid' : '' }}" type="text" name="target" id="target" value="{{ old('target', $banner->target) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('titulo') ? 'is-invalid' : '' }}" type="text" name="titulo" id="titulo" value="{{ old('titulo', $banner->titulo) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('ubicacion') ? 'is-invalid' : '' }}" name="ubicacion" id="ubicacion" required>
                    ...
                    @foreach(App\Models\Banner::UBICACION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('ubicacion', $banner->ubicacion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orden') ? 'is-invalid' : '' }}" type="number" name="orden" id="orden" value="{{ old('orden', $banner->orden) }}" step="1">
                @if($errors->has('orden'))
                    <div class="invalid-feedback">
                        {{ $errors->first('orden') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('estatus') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="estatus" id="estatus" value="1" {{ $banner->estatus || old('estatus', 0) === 1 ? 'checked' : '' }}>
