...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bloques-paginas.update", [$bloquesPagina->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.bloquesPagina.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $bloquesPagina->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pagina') ? 'is-invalid' : '' }}" type="text" name="pagina" id="pagina" value="{{ old('pagina', $bloquesPagina->pagina) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orden') ? 'is-invalid' : '' }}" type="number" name="orden" id="orden" value="{{ old('orden', $bloquesPagina->orden) }}" step="1">
                @if($errors->has('orden'))
                    <div class="invalid-feedback">
                        {{ $errors->first('orden') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('estatus') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="estatus" id="estatus" value="1" {{ $bloquesPagina->estatus || old('estatus', 0) === 1 ? 'checked' : '' }}>
