        ...
        <div class="col-md-12">
            @can('admin_config_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.admin-configs.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $adminConfig->id }}">
                                        ...
                                        <td>
                                            @can('admin_config_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.admin-configs.show', $adminConfig->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('admin_config_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.admin-configs.edit', $adminConfig->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('admin_config_delete')
                                                <form action="{{ route('frontend.admin-configs.destroy', $adminConfig->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('admin_config_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.admin-configs.massDestroy') }}",
