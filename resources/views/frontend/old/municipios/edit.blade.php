    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.municipios.update", [$municipio->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="codigo_municipio">{{ trans('cruds.municipio.fields.codigo_municipio') }}</label>
                            <input class="form-control" type="text" name="codigo_municipio" id="codigo_municipio" value="{{ old('codigo_municipio', $municipio->codigo_municipio) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="codigo_estado_id" id="codigo_estado_id">
                                @foreach($codigo_estados as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('codigo_estado_id') ? old('codigo_estado_id') : $municipio->codigo_estado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $municipio->nombre) }}">
