        ...
        <div class="col-md-12">
            @can('divisa_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.divisas.create') }}">
                            ...
                            <tbody>
                                @foreach($divisas as $key => $divisa)
                                    <tr data-entry-id="{{ $divisa->id }}">
                                        ...
                                        <td>
                                            @can('divisa_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.divisas.show', $divisa->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('divisa_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.divisas.edit', $divisa->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('divisa_delete')
                                                <form action="{{ route('frontend.divisas.destroy', $divisa->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('divisa_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.divisas.massDestroy') }}",
