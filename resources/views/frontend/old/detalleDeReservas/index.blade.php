        ...
        <div class="col-md-12">
            @can('detalle_de_reserva_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.detalle-de-reservas.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $detalleDeReserva->id }}">
                                        ...
                                        <td>
                                            @can('detalle_de_reserva_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.detalle-de-reservas.show', $detalleDeReserva->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('detalle_de_reserva_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.detalle-de-reservas.edit', $detalleDeReserva->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('detalle_de_reserva_delete')
                                                <form action="{{ route('frontend.detalle-de-reservas.destroy', $detalleDeReserva->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('detalle_de_reserva_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.detalle-de-reservas.massDestroy') }}",
