        ...
        <div class="col-md-12">
            @can('estatus_reserva_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.estatus-reservas.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $estatusReserva->id }}">
                                        ...
                                        <td>
                                            @can('estatus_reserva_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.estatus-reservas.show', $estatusReserva->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('estatus_reserva_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.estatus-reservas.edit', $estatusReserva->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('estatus_reserva_delete')
                                                <form action="{{ route('frontend.estatus-reservas.destroy', $estatusReserva->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('estatus_reserva_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.estatus-reservas.massDestroy') }}",
