        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.bloquesPagina.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.bloques-paginas.update", [$bloquesPagina->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="nombre">{{ trans('cruds.bloquesPagina.fields.nombre') }}</label>
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $bloquesPagina->nombre) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="posicion" id="posicion" required>
                                ...
                                @foreach(App\Models\BloquesPagina::POSICION_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('posicion', $bloquesPagina->posicion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="pagina" id="pagina" value="{{ old('pagina', $bloquesPagina->pagina) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="tipo" id="tipo">
                                ...
                                @foreach(App\Models\BloquesPagina::TIPO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo', $bloquesPagina->tipo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('tipo'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="conetenido" id="conetenido">{!! old('conetenido', $bloquesPagina->conetenido) !!}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="orden" id="orden" value="{{ old('orden', $bloquesPagina->orden) }}" step="1">
                            @if($errors->has('orden'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('orden') }}
                                </div>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="estatus" id="estatus" value="1" {{ $bloquesPagina->estatus || old('estatus', 0) === 1 ? 'checked' : '' }}>
                                <label for="estatus">{{ trans('cruds.bloquesPagina.fields.estatus') }}</label>
                            </div>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('frontend.bloques-paginas.storeCKEditorImages') }}', true);
