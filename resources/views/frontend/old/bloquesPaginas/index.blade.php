        ...
        <div class="col-md-12">
            @can('bloques_pagina_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.bloques-paginas.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $bloquesPagina->id }}">
                                        ...
                                        <td>
                                            @can('bloques_pagina_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.bloques-paginas.show', $bloquesPagina->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('bloques_pagina_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.bloques-paginas.edit', $bloquesPagina->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('bloques_pagina_delete')
                                                <form action="{{ route('frontend.bloques-paginas.destroy', $bloquesPagina->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('bloques_pagina_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.bloques-paginas.massDestroy') }}",
